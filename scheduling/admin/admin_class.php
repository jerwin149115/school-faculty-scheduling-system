...
Class Action {
	...
	function save_schedule(){
		...
		$conflictQuery = "SELECT * FROM schedules WHERE location = '$location' 
						  AND ((time_from < '$time_to' AND time_to > '$time_from')";
	
		if(isset($is_repeating)){
			$conflictQuery .= " AND is_repeating = 1 
								AND JSON_CONTAINS(repeating_data, '\"".implode(',', $dow)."\"', '$.dow')";
		} else {
			$conflictQuery .= " AND is_repeating = 0 
								AND schedule_date = '$schedule_date'";
		}
	
		$conflictQuery .= ")";
		
		$conflicts = $this->db->query($conflictQuery);
		
		if($conflicts->num_rows > 0){
			// Get alternative suggestions
			$availableRooms = $this->getAvailableRooms($time_from, $time_to, isset($is_repeating) ? $dow : $schedule_date);
			$availableTimes = $this->getAvailableTimes($location, isset($is_repeating) ? $dow : $schedule_date);
	
			return json_encode([
				'status' => 'conflict',
				'message' => 'Conflict detected with another schedule.',
				'suggestions' => [
		...
		if($save)
			return json_encode(['status' => 'success', 'message' => 'Schedule saved successfully.']);
	}
	
	function getAvailableRooms($time_from, $time_to, $date_or_dow){
		$availableRooms = [];
		$roomsQuery = "SELECT DISTINCT location FROM schedules 
					   WHERE NOT (time_from < '$time_to' AND time_to > '$time_from')";
	
		if(is_array($date_or_dow)){
			$roomsQuery .= " AND is_repeating = 1 
							 AND NOT JSON_CONTAINS(repeating_data, '\"".implode(',', $date_or_dow)."\"', '$.dow')";
		} else {
			$roomsQuery .= " AND is_repeating = 0 
							 AND schedule_date != '$date_or_dow'";
		}
	
		$result = $this->db->query($roomsQuery);
		while($row = $result->fetch_assoc()){
			$availableRooms[] = $row['location'];
		}
	
		return $availableRooms;
	}

	function getAvailableTimes($location, $date_or_dow){
		$availableTimes = [];
		$timeQuery = "SELECT time_from, time_to FROM schedules 
					  WHERE location = '$location' 
					  ORDER BY time_from";
	
		$result = $this->db->query($timeQuery);
		$suggestedStart = "07:00:00"; 
		$suggestedEnd = "21:00:00";   
	
		while($row = $result->fetch_assoc()){
			if($suggestedEnd <= $row['time_from']){
				$availableTimes[] = ['from' => $suggestedStart, 'to' => $row['time_from']];
			}
			$suggestedStart = $row['time_to'];
		}
	
		if($suggestedStart < $suggestedEnd){
			$availableTimes[] = ['from' => $suggestedStart, 'to' => $suggestedEnd];
		}
